...
// Helper to check if a market type is a player prop
function isPlayerProp(marketType) {
  if (!marketType) return false;
  return marketType.startsWith("player_") ||
         ["playerpoints", "playerrebounds", "playerassists", "playerthrees", "playerpra"].includes(marketType.toLowerCase());
}
...
export const BetSlipProvider = ({ children }) => {
  ...
  const addSelection = useCallback((selection) => {
    console.log("[BetSlip] Adding selection:", selection);
    setItems((prev) => {
      ...
      if (existingIndex >= 0) {
        ...
        console.log("[BetSlip] Toggling off existing selection");
        return prev.filter((_, i) => i !== existingIndex);
      }

      // For player props, also check by playerName
      const isPlayerPropSelection = isPlayerProp(selection.marketType);
      const filtered = prev.filter((item) => {
        // Same game, same market type - remove
        if (item.gameId === selection.gameId && item.marketType === selection.marketType) {
          // For player props, also check if it's the same player
          if (isPlayerPropSelection && item.playerName && selection.playerName) {
            return item.playerName !== selection.playerName;
          }
          return false;
        }
        return true;
      ...
      console.log("[BetSlip] Added selection, total items:", filtered.length + 1);
...
function calculateCorrelationFactor(items) {
  ...
  const hasSpread = items.some((i) => i.marketType === "spreads" || i.marketType === "spread");
  const hasTotal = items.some((i) => i.marketType === "totals" || i.marketType === "total");
  const hasPlayerProps = items.some((i) => isPlayerProp(i.marketType));

  // Positive correlation: H2H + Spread for same team
  if (hasH2H && hasSpread) {
    const h2hItem = items.find((i) => i.marketType === "h2h");
    const spreadItem = items.find((i) => i.marketType === "spreads" || i.marketType === "spread");
...
function getConflicts(items) {
  ...
  for (const [gameId, gameItems] of Object.entries(gameGroups)) {
    ...
    const totalItems = gameItems.filter((i) => i.marketType === "total" || i.marketType === "totals");
    ...
    const playerProps = gameItems.filter((i) => isPlayerProp(i.marketType));
    const playerGroups = {};
