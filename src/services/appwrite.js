...
export const createLeague = async (name, userId, displayName, leagueType = "Season") => {
  ...
  await createWeekForLeague(league.$id, leagueType);
  return league;
};
...
export const createWeekForLeague = async (leagueId, leagueType = "Season") => {
  const { startDate, endDate } = getWeekBounds();
  const weekNumber = getWeekNumber();
  const seasonYear = getSeasonYear();

  console.log(`[createWeekForLeague] Creating week ${weekNumber} for league ${leagueId}, type: ${leagueType}`);

  const weekData = {
    leagueId,
    weekNumber: leagueType === "OneOff" ? 1 : weekNumber,
    seasonYear: leagueType === "OneOff" ? 0 : seasonYear,
    startDate: startDate.toISOString(),
    endDate: endDate.toISOString(),
    status: "active", // lowercase to match iOS
    winnerId: "",
  };
  ...
  console.log(`[createWeekForLeague] Created week: ${week.$id}`);
  return week;
};

export const getCurrentWeek = async (leagueId) => {
  // Fetch all weeks for this league
  const allWeeks = await fetchAllDocuments(COLLECTIONS.WEEKS);
  const leagueWeeks = allWeeks.filter((w) => w.leagueId === leagueId);

  console.log(`[getCurrentWeek] Found ${leagueWeeks.length} weeks for league ${leagueId}`);

  // First, look for an ACTIVE week (case-insensitive)
  let week = leagueWeeks.find(
    (w) => w.status?.toLowerCase() === "active"
  );

  if (week) {
    console.log(`[getCurrentWeek] Found active week: ${week.$id}, weekNumber: ${week.weekNumber}`);
    ...
    if (week.endDate) {
      ...
      if (now > endDate) {
        console.log(`[getCurrentWeek] Active week has expired (ended ${endDate.toISOString()})`);

        // Get league to check type
        try {
          const league = await getLeague(leagueId);
          if (league?.leagueType === "OneOff") {
            // OneOff leagues: mark as settled and return it (league is finished)
            console.log(`[getCurrentWeek] OneOff league - marking as settled and returning`);
            await databases.updateDocument(DATABASE_ID, COLLECTIONS.WEEKS, week.$id, {
              status: "settled",
            });
            week.status = "settled";
          } else {
            // Season leagues: mark as settled and create new week
            console.log(`[getCurrentWeek] Season league - marking as settled and creating new week`);
            await databases.updateDocument(DATABASE_ID, COLLECTIONS.WEEKS, week.$id, {
              status: "settled",
            });
          ...
          console.error(`[getCurrentWeek] Error handling expired week:`, err);
        }
      }
    }
  } else {
    // No active week - check for settled week (for OneOff leagues)
    const settledWeek = leagueWeeks.find(
      (w) => w.status?.toLowerCase() === "settled"
    );

    if (settledWeek) {
      console.log(`[getCurrentWeek] Found settled week: ${settledWeek.$id}`);
      // Check if it's a OneOff league
      const league = await getLeague(leagueId);
      if (league?.leagueType === "OneOff") {
        console.log(`[getCurrentWeek] OneOff league with settled week - returning it`);
        week = settledWeek;
      } else {
        // Season league with no active week - create one
        console.log(`[getCurrentWeek] Season league - creating new week`);
        week = await createWeekForLeague(leagueId);
      }
    } else {
      // No weeks at all - create one
      console.log(`[getCurrentWeek] No weeks found - creating new week`);
      week = await createWeekForLeague(leagueId);
    }
  }
  ...
  console.log(`[getCurrentWeek] Found ${balances.length} balances for week ${week.$id}`);
