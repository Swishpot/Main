  ...
  try {
    ...
    if (doc.gamesJson) {
      ...
      if (hoursSinceUpdate < 24) {
        ...
        // Normalize game data - iOS app may use different field names
        return games.map(game => ({
          id: game.id || game.gameId || game.eventId,
          homeTeam: game.homeTeam || game.home_team,
          awayTeam: game.awayTeam || game.away_team,
          homeTeamAbbr: game.homeTeamAbbr || getTeamAbbr(game.homeTeam || game.home_team || ""),
          awayTeamAbbr: game.awayTeamAbbr || getTeamAbbr(game.awayTeam || game.away_team || ""),
          commenceTime: game.commenceTime || game.commence_time || game.startTime,
          markets: game.markets || [],
          isLocked: game.isLocked ?? (game.commenceTime ? new Date(game.commenceTime) < new Date() : false),
        }));
      }
    }
